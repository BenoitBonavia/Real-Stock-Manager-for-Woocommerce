...
* Performance : le marqueur de version installée est désormais chargé avec le reste des réglages au lieu de coûter une requête SQL dédiée à chaque page vue, front compris.

= 3.2.0 =
* Une vignette produit précède désormais chaque ligne de l'onglet « Inventaire » — la même image que sur la fiche produit, avec le même repli automatique (image du produit parent pour une variation qui n'en a pas, puis le placeholder WooCommerce).
...
= 2.1.0 =
* « Besoins Back in Stock » simplifié : la colonne « Inscrits » disparaît, « Disponible » et « Satisfait » (redondants la plupart du temps) fusionnent en une seule colonne.
* Chaque ligne permet d’ajouter directement une quantité au commandé fournisseur — préremplie avec le manque — pour satisfaire la liste d’attente, via le même mécanisme que la commande fournisseur en lot de « Besoins pour commande ».

= 2.0.0 =
* Nouveau module « Besoins Back in Stock » : confronte les inscriptions à une alerte de retour en stock (extension « Back In Stock Notifier for WooCommerce ») au stock libre — déjà en boutique ou commandé au fournisseur — et affiche, référence par référence, combien de demandes seront honorées, combien il en manquerait, et le taux de couverture. Désactivé par défaut, à activer depuis WooCommerce → Réglages → Stocks réels → Modules.
