...
<div class="wrap mh-wrap">
	<h1><?php esc_html_e( 'Besoins & stock', 'real-stock-manager-for-woocommerce' ); ?></h1>
	<p class="description">
		...
		<div class="notice notice-error inline" style="margin:16px 0">
			<p>
				...
				<?php foreach ( $rsmw_unknown as $rsmw_slug ) : ?>
					<span class="mh-slug"><?php echo esc_html( $rsmw_slug ); ?></span>
				<?php endforeach; ?>
	...
	<?php elseif ( ! empty( $rsmw_negatives ) ) : ?>
		<div class="mh-realloc dry">
			<h2><?php esc_html_e( 'Stock négatif hérité', 'real-stock-manager-for-woocommerce' ); ?></h2>
			<p>
				<?php
				printf(
					esc_html(
	...
	<?php if ( $rsmw_realloc ) : ?>
		<div class="mh-realloc <?php echo $rsmw_realloc['dry'] ? 'dry' : ''; ?>">
			<h2>
				<?php
			...
			<?php else : ?>
				<p>
					<strong><?php echo esc_html( (string) (int) $rsmw_realloc['total'] ); ?></strong>
					<?php
					echo $rsmw_realloc['dry']
					...
					?>
					<strong><?php echo esc_html( (string) count( $rsmw_realloc['commandes'] ) ); ?></strong>
					...
					<?php foreach ( $rsmw_realloc['commandes'] as $rsmw_line ) : ?>
						<li>
							...
							<?php if ( $rsmw_line['date'] ) : ?>
								<span class="mh-variant"><?php echo esc_html( sprintf( /* translators: %s: date. */ __( 'du %s', 'real-stock-manager-for-woocommerce' ), $rsmw_line['date'] ) ); ?></span>
							<?php endif; ?>
							...
							<?php if ( ! empty( $rsmw_line['full'] ) ) : ?>
								<span class="mh-full">· <?php esc_html_e( 'complète', 'real-stock-manager-for-woocommerce' ); ?></span>
							<?php endif; ?>
						</li>
					<?php endforeach; ?>
				</ul>
	...
	<?php elseif ( $rsmw_allocatable > 0 ) : ?>
		...
		<div class="mh-realloc">
			<h2><?php esc_html_e( 'Du stock libre attend d’être affecté', 'real-stock-manager-for-woocommerce' ); ?></h2>
			<p>
				<?php
				printf(
					esc_html(
	...
	<div class="mh-kpis">
		<div class="mh-kpi">
			<div class="k"><?php esc_html_e( 'Références concernées', 'real-stock-manager-for-woocommerce' ); ?></div>
			<div class="v" id="k-refs"><?php echo esc_html( (string) count( $rsmw_rows ) ); ?></div>
		</div>
		<div class="mh-kpi">
			<div class="k"><?php esc_html_e( 'Articles à préparer', 'real-stock-manager-for-woocommerce' ); ?></div>
			<div class="v" id="k-restant"><?php echo esc_html( (string) (int) $rsmw_totals['restant'] ); ?></div>
		</div>
		<div class="mh-kpi alert">
			<div class="k"><?php esc_html_e( 'Manquants', 'real-stock-manager-for-woocommerce' ); ?></div>
			<div class="v" id="k-manque"><?php echo esc_html( (string) (int) $rsmw_totals['manque'] ); ?></div>
		</div>
		<div class="mh-kpi alert">
			<div class="k"><?php esc_html_e( 'Références en manque', 'real-stock-manager-for-woocommerce' ); ?></div>
			<div class="v" id="k-refsmanque"><?php echo esc_html( (string) (int) $rsmw_totals['refs_manque'] ); ?></div>
		</div>
		<div class="mh-kpi">
			<div class="k"><?php esc_html_e( 'Valeur du manque', 'real-stock-manager-for-woocommerce' ); ?></div>
			<?php
	...
	<div class="mh-fresh">
		...
		printf(
			...
			'<span class="mh-slug">' . esc_html( $rsmw_status_list ) . '</span>'
		);
		?>
		...
		<?php if ( $rsmw_outside['count'] > 0 ) : ?>
			<div class="mh-outside">
				...
				<?php foreach ( $rsmw_outside['detail'] as $rsmw_index => $rsmw_detail ) : ?>
					...
					<span class="mh-slug"><?php echo esc_html( $rsmw_detail['slug'] ); ?></span>
	...
	<div class="mh-tools">
		...
		<a class="button button-primary" id="mh-export" href="#" data-filename="<?php esc_attr_e( 'besoins-stock', 'real-stock-manager-for-woocommerce' ); ?>">
			<?php esc_html_e( 'Exporter en CSV', 'real-stock-manager-for-woocommerce' ); ?>
		</a>
		<a class="button" href="<?php echo esc_url( $data['stock_page_url'] ); ?>">
			<?php esc_html_e( 'Gestion stock', 'real-stock-manager-for-woocommerce' ); ?>
		</a>
	</div>
	...
	<?php else : ?>
		...
		<table class="mh-table" id="mh-table">
			<thead>
				<tr>
					<th data-key="name"><?php esc_html_e( 'Référence', 'real-stock-manager-for-woocommerce' ); ?></th>
					<th class="mh-num" data-key="demande"><?php esc_html_e( 'Demandé', 'real-stock-manager-for-woocommerce' ); ?></th>
					<th class="mh-num" data-key="pointe"><?php esc_html_e( 'Déjà pointé', 'real-stock-manager-for-woocommerce' ); ?></th>
					<th class="mh-num" data-key="restant"><?php esc_html_e( 'Reste à préparer', 'real-stock-manager-for-woocommerce' ); ?></th>
					<th class="mh-num" data-key="libre"><?php esc_html_e( 'Stock libre', 'real-stock-manager-for-woocommerce' ); ?></th>
					<th class="mh-num" data-key="manque"><?php esc_html_e( 'Manque', 'real-stock-manager-for-woocommerce' ); ?></th>
					<th class="mh-num" data-key="commandes"><?php esc_html_e( 'Commandes', 'real-stock-manager-for-woocommerce' ); ?></th>
					<th class="no-sort"><?php esc_html_e( 'Plus ancienne', 'real-stock-manager-for-woocommerce' ); ?></th>
				</tr>
			</thead>
			<tbody>
				...
				<tr data-search="<?php echo esc_attr( $rsmw_search ); ?>"
					...
					<td>
						...
						<?php if ( '' !== $rsmw_row['variant'] ) : ?>
							<span class="mh-variant"> — <?php echo esc_html( $rsmw_row['variant'] ); ?></span>
						<?php endif; ?>
						<?php if ( '' !== $rsmw_row['sku'] ) : ?>
							<br><span class="mh-sku"><?php echo esc_html( $rsmw_row['sku'] ); ?></span>
						<?php endif; ?>
					</td>
					<td class="mh-num">
						<?php echo $rsmw_row['demande'] ? esc_html( (string) $rsmw_row['demande'] ) : '<span class="mh-zero">·</span>'; ?>
					</td>
					<td class="mh-num">
						<?php echo $rsmw_row['pointe'] ? esc_html( (string) $rsmw_row['pointe'] ) : '<span class="mh-zero">·</span>'; ?>
					</td>
					<td class="mh-num">
						<?php echo $rsmw_row['restant'] ? esc_html( (string) $rsmw_row['restant'] ) : '<span class="mh-zero">·</span>'; ?>
					</td>
					<td class="mh-num <?php echo $rsmw_row['libre'] < 0 ? 'mh-neg' : ''; ?>">
						<?php echo 0 === $rsmw_row['libre'] ? '<span class="mh-zero">·</span>' : esc_html( (string) $rsmw_row['libre'] ); ?>
					</td>
					<td class="mh-num <?php echo $rsmw_row['manque'] > 0 ? 'mh-lack' : 'mh-full'; ?>">
						<?php echo $rsmw_row['manque'] > 0 ? esc_html( (string) $rsmw_row['manque'] ) : '✓'; ?>
					</td>
					<td class="mh-num">
						<?php echo $rsmw_row['commandes'] ? esc_html( (string) $rsmw_row['commandes'] ) : '<span class="mh-zero">·</span>'; ?>
					</td>
					<td>
						<?php if ( $rsmw_row['oldest'] ) : ?>
							<a href="<?php echo esc_url( $rsmw_row['oldest']['url'] ); ?>">#<?php echo esc_html( $rsmw_row['oldest']['num'] ); ?></a>
							<span class="mh-variant"><?php echo esc_html( $rsmw_row['oldest']['date'] ); ?></span>
						<?php else : ?>
		...
		<div class="mh-empty" id="mh-noresult" style="display:none">
	...
	<div class="mh-note">
		...
		<?php foreach ( wc_get_order_statuses() as $rsmw_slug => $rsmw_label ) : ?>
			...
			?>
			<span class="mh-slug <?php echo $rsmw_active ? 'is-active' : ''; ?>"><?php echo esc_html( $rsmw_clean ); ?></span>
			<?php echo esc_html( $rsmw_label ); ?>
			&nbsp;·&nbsp;
		<?php endforeach; ?>
		<br><br>
		<?php esc_html_e( 'Les slugs surlignés sont ceux actuellement pris en compte. Cette page recalcule à chaque affichage ; le cache ne concerne que les indicateurs des fiches produit.', 'real-stock-manager-for-woocommerce' ); ?>

		<br><br>
		<strong><?php esc_html_e( 'Attribution automatique', 'real-stock-manager-for-woocommerce' ); ?></strong> —
		<?php if ( $data['auto_allocate'] ) : ?>
			<span class="mh-full"><?php esc_html_e( 'activée', 'real-stock-manager-for-woocommerce' ); ?></span> :
		...
		<?php else : ?>
			<span class="mh-lack"><?php esc_html_e( 'désactivée', 'real-stock-manager-for-woocommerce' ); ?></span> —
		...
		<?php echo $data['status_ok'] ? '<span class="mh-full">' . esc_html__( 'oui', 'real-stock-manager-for-woocommerce' ) . '</span>' : '<span class="mh-lack">' . esc_html__( 'non', 'real-stock-manager-for-woocommerce' ) . '</span>'; ?>
		·
		<?php esc_html_e( 'déclaré à WooCommerce :', 'real-stock-manager-for-woocommerce' ); ?>
		<?php echo $data['status_declared'] ? '<span class="mh-full">' . esc_html__( 'oui', 'real-stock-manager-for-woocommerce' ) . '</span>' : '<span class="mh-lack">' . esc_html__( 'non', 'real-stock-manager-for-woocommerce' ) . '</span>'; ?>
		·
